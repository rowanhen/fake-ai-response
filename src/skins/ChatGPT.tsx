    ...
    <div className={`${bg} ${text} min-h-[400px] rounded-lg overflow-auto ${scrollbar}`}>
            ...
            {message.role === 'user' ? (
              ...
              <div className="space-y-1">
                <div className="flex justify-end">
                  <div className={`${userBubble} max-w-[85%] rounded-3xl px-4 py-2.5`}>
                    <div className="whitespace-pre-wrap leading-[1.6]">{message.content}</div>
                  </div>
                </div>
      ...
      <div className={`sticky bottom-0 ${bg} px-4 py-4`}>
        <div className="max-w-3xl mx-auto">
          <div className={`${inputBg} rounded-3xl px-3 py-2.5 flex items-center gap-3`}>
            <button className={`p-1.5 rounded-full ${darkMode ? 'bg-[#424242]' : 'bg-[#e8e8e8]'} flex-shrink-0`}>
            ...
            <div className="flex items-center gap-1.5">
              <button className={`p-2 rounded-full ${darkMode ? 'bg-[#424242]' : 'bg-[#e8e8e8]'} flex-shrink-0`}>
              ...
              <button className={`p-2 rounded-full ${darkMode ? 'bg-[#424242]' : 'bg-[#e8e8e8]'} flex-shrink-0`}>
                <svg className="w-4 h-4" viewBox="0 0 24 24" fill={darkMode ? '#e0e0e0' : '#555'}>
                  <rect x="3" y="8" width="3" height="8" rx="1.5" />
                  <rect x="8.5" y="5" width="3" height="14" rx="1.5" />
