        ...
        {messages.map((message) => (
          <div key={message.id} className="flex gap-4">
            ...
            <div className="flex-shrink-0">
              {message.role === 'user' ? (
                <div className={`w-8 h-8 rounded-full ${darkMode ? 'bg-[#5436DA]' : 'bg-[#5436DA]'} flex items-center justify-center text-white text-sm font-medium`}>
                  U
                </div>
              ) : (
                <div className={`w-8 h-8 rounded-full ${darkMode ? 'bg-[#19c37d]' : 'bg-[#19c37d]'} flex items-center justify-center`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className={`whitespace-pre-wrap leading-relaxed ${message.role === 'assistant' ? 'prose prose-sm max-w-none' : ''}`}>
                {message.content}
              </div>
            </div>
          </div>
      ...
      <div className={`sticky bottom-0 ${bg} border-t ${border} p-4`}>
        <div className="max-w-3xl mx-auto">
          <div className={`${assistantBg} rounded-2xl px-4 py-3 flex items-center gap-2`}>
            <span className={`flex-1 ${darkMode ? 'text-[#8e8ea0]' : 'text-[#8e8ea0]'}`}>Message ChatGPT</span>
